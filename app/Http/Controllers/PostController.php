...
class PostController extends Controller
{
    ...
    {
        return view('posts.all', compact('user'));
    }

    public function getAllPosts(Request $request, User $user)
    {
        $query = Post::with('user')->where('user_id', $user->id);
    ...
    {
        $user = auth()->user();
        $posts = Post::where('user_id', $user->id)->get(); // Ambil semua postingan user

        return ApiFormatter::sendResponse('success', 200, 'Profile retrieved successfully.', [
            'user' => $user,
            'posts' => $posts,
        ]);
    }
